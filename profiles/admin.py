...
class UserMessageInline(admin.TabularInline):
    model = UserMessage
    extra = 0
    readonly_fields = ('user', 'created_by', 'content', 'created_at')
    ordering = ('created_at',)
...
class LoyaltyPointsTransactionInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = ('user_profile', 'transaction_type', 'points', 'balance_before', 'balance_after', 'created_at')
    ordering = ('created_at',)
...
class UserMessageAdmin(admin.ModelAdmin):
    list_display = ('user', 'created_by', 'content', 'created_at', 'repair_request', 'contact_message')
